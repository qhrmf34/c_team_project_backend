...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 기본OAuth2 처리(Kakao, Naver 등)
        OAuth2User oauth2User = super.loadUser(userRequest);

        // 기존 회원만 로그인, 신규 회원은 임시JWT만
        ...
        try {
            ...
            if (loginResponse != null) {
                if (loginResponse.getMemberId() != null) {
                    // 기존 회원- 정식 로그인
                    System.out.println("✓ 기존 회원 로그인 성공! 회원ID: " + loginResponse.getMemberId());
                    System.out.println("✓ JWT 토큰 생성 완료: " + loginResponse.getToken().substring(0, 20) + "...");
                } else {
                    // 신규 회원- 임시JWT만 발급(DB 저장 안 함)
                    System.out.println("✓ 신규 회원- 임시JWT 발급 완료(DB 미저장)");
                    System.out.println("✓ 임시JWT 토큰: " + loginResponse.getToken().substring(0, 20) + "...");
                }
                ...
                System.out.println("✓ 세션에LoginResponse 저장 완료");
            }
